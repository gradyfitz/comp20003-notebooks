...
def ass2_part_one(search_point, zoom_on_found):
    ...
    program = subprocess.Popen(['bash', '-c', "cd submission && mkdir results && echo {} > results/stdin && rm *.o && make -B map1 && /bin/bash ./map1 /home/shared/ass2/clue.csv results/outfile < results/stdin".format(" ".join([str(n) for n in search_point]))], 
...
def ass2_part_two(search_point, radius, zoom_on_found):
    program = subprocess.Popen(['bash', '-c', "cd submission && mkdir results && echo {} > results/stdin && rm *.o && make -B map2 && /bin/bash ./map2 /home/shared/ass2/clue.csv results/outfile < results/stdin".format("{} {}".format(" ".join([str(n) for n in search_point]), radius))], 
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    timelog(" - stdout - ")
    print(program.stdout.read().decode())
    timelog(" - stderr - ")
